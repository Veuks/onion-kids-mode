// super-kidui - unified games and videos carousel for Onion OS
//
...
#define REMAINING_FILE "/tmp/kidsmode_v2_remaining"
#define RESULT_FILE "/tmp/kidsmode_v2_ui_result"
#define VIDEOS_DIR "/mnt/SDCARD/Media/SuperKidsMode"
#define FAVORITES_PATH "/mnt/SDCARD/Roms/favourite.json"
#define ICON_X_PATH "/mnt/SDCARD/App/SuperKidsMode/icon-X-54.png"
#define SCREEN_REFLECTION_PATH \
    "/mnt/SDCARD/App/SuperKidsMode/screen-reflection.png"
...
// (App/SuperKidsMode/icon-X-54.png)
...
static void loadFavorites(void)
{
    ...
    while (games_count < MAX_GAMES && fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (strstr(entry.launch, "/App/KidsMode/") != NULL ||
            strstr(entry.launch, "/App/KidsModeV2/") != NULL ||
            strstr(entry.launch, "/App/SuperKidsMode/") != NULL ||
...
static void renderEmpty(void)
{
    ...
    drawText(videos ? "Add videos to Media/SuperKidsMode"
...
{
    renderBase();
    theme_renderHeader(screen, "Super Kids Mode", false);
...
{
    renderBase();
    theme_renderHeader(screen, "Super Kids Mode - Parent Menu", false);
...
static void renderPin(const char *title, bool show_intro)
{
    ...
    if (show_intro) {
        drawText("Super Kids Mode shows favorite games", cx,
...
int main(int argc, char *argv[])
{
    ...
    list_addItem(&menu_list,
                 (ListItem){.label = "Exit Super Kids Mode",
                            .item_type = ACTION});
    ...
    if (set_pin_mode) {
        ...
        loadCurrentFloor();
        fprintf(stderr, "super-kidui: loaded %d entries on %s floor\n",
                games_count,
                current_floor == FLOOR_VIDEOS ? "VIDEOS" : "GAMES");
        rememberSelection();
        writeSelectionState();
