// kidui-deluxe - unified games and videos carousel for Onion OS
//
...
#define ICON_X_PATH "/mnt/SDCARD/App/KidsModeDeluxe/icon-X-54.png"
#define SCREEN_REFLECTION_PATH \
    "/mnt/SDCARD/App/KidsModeDeluxe/screen-reflection.png"
...
// (App/KidsModeDeluxe/icon-X-54.png)
...
static void loadFavorites(void)
{
    ...
    while (games_count < MAX_GAMES && fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (strstr(entry.launch, "/App/KidsMode/") != NULL ||
            ...
            strstr(entry.launch, "/App/KidsModeDeluxe/") != NULL ||
...
static void renderEmpty(void)
{
    ...
    drawText(videos ? "Add videos to Media/VideoKidsMode"
...
{
    renderBase();
    theme_renderHeader(screen, "Kids Mode Deluxe - Parent Menu", false);
...
static void renderPin(const char *title, bool show_intro)
{
    ...
    if (show_intro) {
        drawText("Kids Mode Deluxe shows favorite games", cx,
...
int main(int argc, char *argv[])
{
    ...
    list_addItem(&menu_list,
                 (ListItem){.label = "Exit Kids Mode Deluxe",
                            .item_type = ACTION});
    ...
    if (set_pin_mode) {
        ...
        loadCurrentFloor();
        fprintf(stderr, "kidui-deluxe: loaded %d entries on %s floor\n",
                games_count,
                current_floor == FLOOR_VIDEOS ? "VIDEOS" : "GAMES");
        rememberSelection();
        writeSelectionState();
