...
/* Keep enough decoded audio queued for the Miyoo's modest CPU. At 44.1/48 kHz,
 * ten callbacks per second rounds up to an 8192-sample SDL buffer. This gives
 * demanding video frames more headroom without changing decoded audio. */
...
/* Emit one compact line at shutdown; never write continuously to the SD card. */
static void kidsplay_log_audio_diagnostics(void)
{
    ...
    fprintf(stderr,
            ...
            "decode_misses=%u buffer_samples=%d\n",
            kidsplay_audio_callbacks, kidsplay_audio_late_callbacks,
            kidsplay_audio_decode_misses, kidsplay_audio_buffer_samples);
    fflush(stderr);
}
...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    /* The Miyoo SDL2 renderer has no planar or packed YUV upload path.
     * Its prebuilt Mini renderer also submits copied textures to MI GFX as
     * ARGB8888 regardless of SDL's declared format, so the fallback must
     * remain ARGB8888 for correct pixels on real hardware. */
    ...
    switch (sdl_pix_fmt) {
        ...
        case SDL_PIXELFORMAT_IYUV:
            ...
            break;
        default:
            /* SDL_UpdateTexture in the Mini renderer keeps the caller's
             * pointer instead of copying it as SDL promises. FFmpeg may
             * recycle that AVFrame and its aligned stride can exceed the
             * visible width. Copy each row into SDL's own compact, persistent
             * texture allocation, exactly like the working display test. */
            {
                uint8_t *dst_pixels = NULL;
                int dst_pitch = 0;
                int bytes_per_pixel = SDL_BYTESPERPIXEL(sdl_pix_fmt);
                int row_bytes = frame->width * bytes_per_pixel;
                int y;
                ret = SDL_LockTexture(*tex, NULL, (void **)&dst_pixels,
                                      &dst_pitch);
                if (ret == 0) {
                    for (y = 0; y < frame->height; y++) {
                        int source_y = frame->linesize[0] < 0
                                           ? frame->height - 1 - y
                                           : y;
                        const uint8_t *source = frame->data[0] +
...
static void video_image_display(VideoState *is)
{
    ...
    /* The Miyoo Mini renderer's MI_GFX copy path does not reliably scale a
     * texture to a differently-sized destination rectangle.  The isolated
     * hardware test proves that a 640x480 texture copied 1:1 is presented.
     * The test launcher already converts the frame to 640x480, so keep this
     * final copy strictly 1:1 as well. */
    rect.x = 0;
    rect.y = 0;
    rect.w = 640;
    rect.h = 480;
...
static int video_open(VideoState *is)
{
    ...
    /* The Mini video backend only implements window creation; its later
     * resize/fullscreen callbacks are absent.  Create the final fullscreen
     * window once in main() and do not mutate it here. */
...
static void video_display(VideoState *is)
{
    ...
    if (getenv("VC_MEDIA_KIND") &&
        !strcmp(getenv("VC_MEDIA_KIND"), "audio"))
        kp_audio_display(is);
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    if (kidsplay_audio_previous_callback > 0 &&
        kidsplay_audio_expected_callback > 0 &&
        callback_now - kidsplay_audio_previous_callback >
            kidsplay_audio_expected_callback * 3 / 2)
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        ...
        case SDL_KEYUP:
            if (getenv("VC_MEDIA_KIND")) {
                kp_handle_key(cur_stream, &event.key);
                break;
            }
