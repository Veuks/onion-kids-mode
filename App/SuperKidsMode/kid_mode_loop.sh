#!/bin/sh
# ---------------------------------------------------------------------------
# Super Kids Mode for Onion OS — arming, play loop, and unlock logic.
#
...
appdir=/mnt/SDCARD/App/SuperKidsMode
...
# App/SuperKidsMode during an update can never delete them.
backupdir=/mnt/SDCARD/Saves/SuperKidsMode
...
logfile=/mnt/SDCARD/.tmp_update/logs/superkidsmode.log
...
remaining_file=/tmp/kidsmode_v2_remaining
...
uiresult=/tmp/kidsmode_v2_ui_result
lockfloor_result=/tmp/kidsmode_v2_lockfloor_result
uilog=/tmp/kidmode_ui_log

# Unified V2 video state. Game state continues to use Onion's native
...
videosdir=/mnt/SDCARD/Media/SuperKidsMode
v2state="$backupdir/v2_state.json"
positions="$backupdir/video_positions"
ffplay=/mnt/SDCARD/.tmp_update/bin/ffplay
player_pid=/tmp/kidsmode_v2_player.pid
menu_exit_marker=/tmp/videocarousel_menu_exit
libvcinput="$appdir/bin/libvcinput.so"
game_selection_file="$backupdir/v2_game_selection.txt"
video_selection_file="$backupdir/v2_video_selection.txt"
series_selections_dir="$backupdir/v2_series_selections"
...
# back from the snapshot in Saves/SuperKidsMode. Returns 0 if a PIN is on file
# afterwards.
restore_pin_backup() {
...
# arm, so installing Super Kids Mode is just copying App/SuperKidsMode
...
# Adds a "Super Kids Mode" entry to Onion's Favorites tab (usually the
# boot tab),
...
fav_entry='{"label":"Super Kids Mode","launch":"/mnt/SDCARD/App/SuperKidsMode/launch.sh","type":5,"imgpath":"/mnt/SDCARD/Icons/Default/app/guest_on.png","rompath":"/mnt/SDCARD/App/SuperKidsMode/launch.sh"}'
...
ensure_fav_shortcut() {
    ...
    # Remove the obsolete tester shortcut without touching the separate
    # original Kids Mode V1 app if it is still installed.
    if grep -qF "/App/KidsModeV2/launch.sh" "$favfile" 2> /dev/null; then
        awk 'index($0, "/App/KidsModeV2/launch.sh") == 0 { print }' \
    ...
    if [ "$(config_get fav_shortcut)" != "true" ]; then
        if grep -qF "/App/SuperKidsMode/launch.sh" "$favfile" 2> /dev/null; then
            awk 'index($0, "/App/SuperKidsMode/launch.sh") == 0 { print }' \
                "$favfile" > "$favfile.tmp" && mv -f "$favfile.tmp" "$favfile"
            log "Removed Super Kids Mode shortcut from favorites."
        fi
        return 0
    fi
    ...
    if ! grep -qF "/App/SuperKidsMode/launch.sh" "$favfile" 2> /dev/null; then
        ...
        printf '%s\n' "$fav_entry" >> "$favfile"
        log "Added Super Kids Mode shortcut to favorites."
    fi
}
...
parent_menu() {
    while :; do
        ...
        rm -f "$uiresult" /tmp/kidsmode_v2_game_selection \
            /tmp/kidsmode_v2_video_selection
        case "$menu_action" in
            UNLOCK)
                return 0
                ;;
...
disarm() {
    ...
    sync
    log "Super Kids Mode disarmed."
    infoPanel -t "Super Kids Mode" -m "Unlocked!\nReturning to Onion." --auto
...
v2_get() {
    jq -r "$1 // empty" "$v2state" 2> /dev/null
}

v2_save() {
    v2_floor="$1" v2_mode="$2" v2_video="$3" v2_folder="$4"
    mkdir -p "$backupdir" "$positions" "$videosdir/Imgs"
    jq -n --arg floor "$v2_floor" --arg mode "$v2_mode" \
        --arg video "$v2_video" --arg folder "$v2_folder" \
        '{version:2,active_floor:$floor,active_mode:$mode,last_video:$video,active_folder:$folder}' \
        > "$v2state.tmp" && mv -f "$v2state.tmp" "$v2state"
}
...
restore_ffplay_state() {
    for backup in $(find /mnt/SDCARD/App/FFplay "$sysdir" \
        -name 'pos.cfg.kidsmode-v2-backup' 2> /dev/null); do
        original="${backup%.kidsmode-v2-backup}"
        rm -f "$original"
...
hide_ffplay_state() {
    ...
    for original in $(find /mnt/SDCARD/App/FFplay "$sysdir" \
        ...
        mv -f "$original" "$original.kidsmode-v2-backup"
    done
}
...
play_video_v2() {
    ...
    runtime_pos="/tmp/kidsmode_v2_position.$$"
    start=0
    ...
    [ "$fresh" = yes ] && printf '0\n' > "$posfile"
    printf '%s\n' "$start" > "$runtime_pos"
    v2_save videos running "$video" "$active_folder"
    hide_ffplay_state
    ...
    rm -f "$menu_exit_marker"
    v2_save videos carousel "$video" "$active_folder"
}
...
cmd_run() {
    ...
    active_floor="$(v2_get '.active_floor')"
    [ "$active_floor" = videos ] || active_floor=games
    active_folder="$(v2_get '.active_folder')"
    last_video="$(v2_get '.last_video')"
    ...
    if [ "$(v2_get '.active_mode')" = running ] && [ -f "$last_video" ] &&
        [ "$(timer_remaining)" != 0 ]; then
        active_floor=videos
        play_video_v2 "$last_video" no
    fi
    ...
    if [ -f "$sysdir/cmd_to_run.sh" ] && is_game_cmd "$sysdir/cmd_to_run.sh"; then
        if [ "$(timer_remaining)" = "0" ]; then
            ...
            active_floor=games
            run_game_cmd
            v2_save games carousel "$last_video" "$active_folder"
        fi
    elif [ "$(v2_get '.active_mode')" = running ] &&
        [ "$active_floor" = games ] && [ -f "$last_game_file" ] &&
        [ "$(timer_remaining)" != 0 ]; then
        # Some shutdown paths let the emulator wrapper return and remove
        # cmd_to_run.sh just before the hardware actually powers off. The V2
        ...
        if [ -f "$lg_launch" ] && [ -f "$lg_rompath" ]; then
            log "resuming interrupted game from V2 state: $lg_rompath"
    ...
    while [ -f "$flagfile" ]; do
        ...
        ui_floor="$(cat /tmp/kidsmode_v2_floor 2> /dev/null)"
        case "$ui_floor" in games | videos) active_floor="$ui_floor" ;; esac
        ui_folder="$(sed -n 1p /tmp/kidsmode_v2_folder 2> /dev/null)"
        case "$ui_folder" in
            "$videosdir"/*) [ -d "$ui_folder" ] && active_folder="$ui_folder" ;;
            '') active_folder="" ;;
        esac
        ui_selection="$(sed -n 1p /tmp/kidsmode_v2_selection 2> /dev/null)"
        ui_game_selection="$(sed -n 1p /tmp/kidsmode_v2_game_selection 2> /dev/null)"
        ui_video_selection="$(sed -n 1p /tmp/kidsmode_v2_video_selection 2> /dev/null)"
        [ -n "$ui_game_selection" ] &&
        ...
        v2_save "$active_floor" carousel "$last_video" "$active_folder"
        ...
        case "$ui_rc" in
            0) # game selected
                ...
                case "$sel_verb" in
                    FOLDER)
                        ...
                        last_video=""
                        v2_save videos carousel "" "$active_folder"
                        continue
                        ;;
                    BACK)
                        active_floor=videos
                        ...
                        active_folder=""
                        printf '%s\n' "$folder_selection" > "$video_selection_file"
                        v2_save videos carousel "$last_video" ""
                        continue
                        ;;
                    PLAY | RESTART)
                        active_floor=videos
                        ...
                        play_video_v2 "$last_video" "$([ "$sel_verb" = RESTART ] && echo yes || echo no)"
                        ui_fails=0
                        continue
                        ;;
                ...
                v2_save games running "$last_video" "$active_folder"
                run_game_cmd
                v2_save games carousel "$last_video" "$active_folder"
                ui_fails=0
                ;;
            ...
            *) # UI crashed or won't start
                ...
                if [ "$ui_fails" -ge 3 ]; then
                    ...
                    infoPanel -t "Super Kids Mode" -m "Interface failed.\nReturning to normal Onion." --auto
                    disarm
                    return 1
                fi
                sleep 1
...
cmd_arm() {
    if [ ! -f "$kidui_bin" ]; then
        infoPanel -t "Super Kids Mode" -m "kidui binary is missing.\nReinstall Super Kids Mode." --auto
        return 1
    fi
    ...
    if [ "$fav_count" -eq 0 ]; then
        infoPanel -t "Super Kids Mode" -m "No favorites found.\nAdd some favorites first,\nthen try again." --auto
        return 1
    fi

    if ! install_hook; then
        infoPanel -t "Super Kids Mode" -m "kidmode_boot.sh is missing.\nReinstall Super Kids Mode." --auto
        return 1
    fi

    if ! ensure_pin; then
        infoPanel -t "Super Kids Mode" -m "PIN setup canceled.\nThe mode was not armed." --auto
        return 1
    fi
    ...
    (sleep 2; sync) &
    log "Super Kids Mode armed (timer: $(get_timer_minutes) min)."
...
case "${1:-run}" in
    ...
    run)
        ...
        # back from the snapshot in Saves/SuperKidsMode
        has_pin || restore_pin_backup
        cmd_run
        ;;
    *)
