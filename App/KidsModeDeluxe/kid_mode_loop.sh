#!/bin/sh
# ---------------------------------------------------------------------------
# Kids Mode Deluxe for Onion OS — arming, play loop, and unlock logic.
#
...
appdir=/mnt/SDCARD/App/KidsModeDeluxe
...
ensure_pin() {
    ...
    setup_notice=""
    while :; do
        pin1="$(run_pin_entry "Set Deluxe PIN" "$setup_notice")" || return 1
...
# arm, so installing Kids Mode Deluxe is just copying App/KidsModeDeluxe
...
# Adds a "Kids Mode Deluxe" entry to Onion's Favorites tab (usually the
# boot tab),
...
fav_entry='{"label":"Kids Mode Deluxe","launch":"/mnt/SDCARD/App/KidsModeDeluxe/launch.sh","type":5,"imgpath":"/mnt/SDCARD/Icons/Default/app/guest_on.png","rompath":"/mnt/SDCARD/App/KidsModeDeluxe/launch.sh"}'
...
ensure_fav_shortcut() {
    ...
    if [ "$(config_get fav_shortcut)" != "true" ]; then
        if grep -qF "/App/KidsModeDeluxe/launch.sh" "$favfile" 2> /dev/null; then
            awk 'index($0, "/App/KidsModeDeluxe/launch.sh") == 0 { print }' \
                "$favfile" > "$favfile.tmp" && mv -f "$favfile.tmp" "$favfile"
            log "Removed Kids Mode Deluxe shortcut from favorites."
        fi
        return 0
    fi
    ...
    if ! grep -qF "/App/KidsModeDeluxe/launch.sh" "$favfile" 2> /dev/null; then
        ...
        printf '%s\n' "$fav_entry" >> "$favfile"
        log "Added Kids Mode Deluxe shortcut to favorites."
    fi
}
...
disarm() {
    ...
    sync
    log "Kids Mode Deluxe disarmed."
    infoPanel -t "Kids Mode Deluxe" -m "Unlocked!\nReturning to Onion." --auto
        ...
        case "$ui_rc" in
            ...
            *) # UI crashed or won't start
                ...
                if [ "$ui_fails" -ge 3 ]; then
                    ...
                    infoPanel -t "Kids Mode Deluxe" -m "Interface failed.\nReturning to normal Onion." --auto
                    disarm
                    return 1
                fi
                sleep 1
...
cmd_arm() {
    if [ ! -f "$kidui_bin" ]; then
        infoPanel -t "Kids Mode Deluxe" -m "kidui binary is missing.\nReinstall KidsModeDeluxe." --auto
        return 1
    fi
    ...
    if [ "$fav_count" -eq 0 ]; then
        infoPanel -t "Kids Mode Deluxe" -m "No favorites found.\nAdd some favorites first,\nthen try again." --auto
        return 1
    fi

    if ! install_hook; then
        infoPanel -t "Kids Mode Deluxe" -m "kidmode_boot.sh is missing.\nReinstall KidsModeDeluxe." --auto
        return 1
    fi

    if ! ensure_pin; then
        infoPanel -t "Kids Mode Deluxe" -m "PIN setup canceled.\nThe mode was not armed." --auto
        return 1
    fi
    ...
    (sleep 2; sync) &
    log "Kids Mode Deluxe armed (timer: $(get_timer_minutes) min)."
